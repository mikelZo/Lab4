...
public class Colosseum {
    ...
    public static Pokemon buildPokemon() {
        ...
        if (x <= MAX_HIT_POINTS && x >= 1){
            tempPokemon.hitPoints = x;
        } else {
            while (x > MAX_HIT_POINTS || x < 1){
        ...
        if (y < MAX_HIT_POINTS && y >= 1){
            tempPokemon.attackLevel = y;
        } else {
            while (y > MAX_HIT_POINTS || myScan.nextInt() < 1){
        ...
        if (z == MAX_HIT_POINTS - tempPokemon.attackLevel){
            tempPokemon.defenseLevel = z;
        } else {
            while (z != MAX_HIT_POINTS - tempPokemon.attackLevel){
                System.out.println("Please enter remaining points in defense");
                z = myScan.nextInt();
            }
            tempPokemon.defenseLevel = z;
        }
